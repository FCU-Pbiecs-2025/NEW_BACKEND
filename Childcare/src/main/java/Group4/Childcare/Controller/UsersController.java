...
public class UsersController {
    ...
    public ResponseEntity<Map<String, Object>> createOrUpdateUserJdbc(@RequestBody Users user) {
        ...
        try {
            ...
            // 檢查帳號是否已存在（只在新增時檢查，userID 為 null 表示新增）
            if (user.getUserID() == null) {
                if (usersService.isAccountExists(user.getAccount())) {
                    result.put("success", false);
                    result.put("code", 400);
                    result.put("error", "Account already exists");
    ...
    /**
     * GET /users/check-account/{account}
     * 檢查帳號是否已存在
     *
    ...
    public ResponseEntity<Boolean> checkAccountExists(@PathVariable String account) {
        try {
            if (account == null || account.trim().isEmpty()) {
                return ResponseEntity.badRequest().body(false);
            }
            boolean exists = usersService.isAccountExists(account);
            return ResponseEntity.ok(exists);
        } catch (Exception e) {
            System.err.println("Error in checkAccountExists: " + e.getMessage());
            e.printStackTrace();
